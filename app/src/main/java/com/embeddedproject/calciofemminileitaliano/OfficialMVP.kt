...
class OfficialMVP : Fragment() {
    ...
    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?, savedInstanceState: Bundle?): View? {
        return inflater.inflate(R.layout.fragment_official_mvp, container, false)
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val arguments = OfficialMVPArgs.fromBundle(requireArguments())
        ...
        view.findViewById<ImageView>(R.id.back_to_championship_prediction).setOnClickListener {
            val navigateToMatchesPredictions = OfficialMVPDirections.actionOfficialMVPToMatchesPredictions(user, championship, season)
        ...
        view.findViewById<ImageView>(R.id.logout).setOnClickListener {
            ...
            builder.setPositiveButton(getString(R.string.confirm)) { dialog, _ ->
                ...
                val navigateToLoginRegistration = OfficialMVPDirections.actionOfficialMVPToLoginRegistration()
        ...
        val homeResult: TextView = view.findViewById(R.id.home_real_result)
        val guestResult: TextView = view.findViewById(R.id.guest_real_result)
        ...
        databaseMatch.child("MatchInfo").get().addOnCompleteListener {
            ...
            homeResult.text = homeScore
            guestResult.text = guestScore
            ...
            reference.child("Players").child(season).get().addOnCompleteListener { players ->
                ...
                val officialMVPReference = databaseMatch.child("OfficialMVP")
                val playersAdapter = PlayerMVPAdapter(view.context, playersList, officialMVPReference, homeBitmap!!, guestBitmap!!, homeTeam)
                listViewPlayers.adapter = playersAdapter
                view.findViewById<ProgressBar>(R.id.progress_updating_players).visibility = INVISIBLE

                officialMVPReference.get().addOnCompleteListener { mvp ->
                    var selectedPosition = -1
                    if (mvp.result.value.toString() != "null") {
                        val teamMVPPredicted = mvp.result.child("team").value.toString()
                        val shirtMVPPredicted = mvp.result.child("shirt").value.toString().toInt()
